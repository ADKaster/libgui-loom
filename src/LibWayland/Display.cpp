...
NonnullOwnPtr<Callback> Display::sync()
{
    auto* callback = wl_display_sync(m_display);
    return adopt_own(*new Callback(callback));
}

Registry& Display::registry() const
{
    if (!m_registry) {
        auto* wayland_registry = wl_display_get_registry(m_display);
        m_registry = adopt_own(*new Registry(wayland_registry));
    }
    return *m_registry;
}
