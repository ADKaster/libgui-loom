...
bool ScreenLayout::try_auto_add_display_connector(ByteString const& device_path)
{
    ...
    struct CocoaScreenInfo {
        ByteString identifier;
        Gfx::IntRect rect;
        int scale_factor;
    };

    Vector<CocoaScreenInfo, 8> cocoa_screens;
    for (NSScreen* screen in [NSScreen screens]) {
        NSNumber* screen_number = [screen.deviceDescription objectForKey:@"NSScreenNumber"];
        if (!screen_number)
            continue;

        auto cocoa_scale_factor = [screen backingScaleFactor];
        if (cocoa_scale_factor < 1)
            cocoa_scale_factor = 1;
        int screen_scale_factor = max(1, static_cast<int>(cocoa_scale_factor + 0.5));

        auto frame = [screen frame];
        int x = static_cast<int>(frame.origin.x);
        int y = static_cast<int>(frame.origin.y);
        int width = max(1, static_cast<int>(frame.size.width));
        int height = max(1, static_cast<int>(frame.size.height));

        cocoa_screens.append({
            .identifier = ByteString::formatted("CGDisplay{}", [screen_number unsignedIntValue]),
            .rect = { x, y, width, height },
            .scale_factor = screen_scale_factor,
        });
    }

    if (cocoa_screens.is_empty()) {
        dbgln("Failed to query displays from Cocoa");
        return false;
    }

    auto has_device = [&](ByteString const& candidate_device) {
        for (auto const& screen : screens) {
            if (!screen.device.has_value())
                continue;
            if (screen.device.value() == candidate_device)
                return true;
        }
        return false;
    };

    Optional<CocoaScreenInfo> selected_screen;
    if (!device_path.is_empty()) {
        for (auto const& cocoa_screen : cocoa_screens) {
            if (cocoa_screen.identifier != device_path)
                continue;
            if (has_device(cocoa_screen.identifier))
                break;
            selected_screen = cocoa_screen;
            break;
        }
    }
    ...
    if (!selected_screen.has_value()) {
        dbgln("No new Cocoa display found to add for device {}", device_path);
        return false;
    }
    ...
    screens.append({
        .mode = Screen::Mode::Device,
        .device = selected_screen->identifier,
        .location = selected_screen->rect.location(),
        .resolution = selected_screen->rect.size(),
        .scale_factor = selected_screen->scale_factor,
    });

    normalize();
    if (!is_valid()) {
        dbgln("Failed to add Cocoa display {} to screen layout", selected_screen->identifier);
        return false;
    }
